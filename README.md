...
php artisan migrate --seed   # creates the schema AND a test user (see below)
```
...
`php artisan db:seed` (run automatically by `migrate --seed` above) creates
a deterministic test user with a small set of sample notes, so you can
start exercising the API immediately without having to register first.

| Field    | Value               |
|----------|---------------------|
| Email    | `test@example.com`  |
| Password | `password`          |
| Notes    | 10 (3 named + 7 generated by the factory) |

The seeder is idempotent — re-running it is a no-op once the test user
exists. To wipe the DB and re-seed from scratch:

```bash
php artisan migrate:fresh --seed
```

If you'd rather start with an empty database, drop the `--seed` flag from
`php artisan migrate` in any of the platform-specific commands below.
...
4. Runs `php artisan migrate --seed` (creates the test user described above)
...
If you ran `migrate --seed`, the fastest way to play with the API is to
log in as the seeded test user and reuse its token. Otherwise, register
your own account with `POST /api/register`.
...
# 1. Log in as the seeded test user — grab the token from the response
TOKEN=$(
...
# 2. List the seeded notes (paginated, with optional ?search=)
curl -s "$BASE/notes?per_page=5" \
...
curl -s -X POST "$BASE/notes" \
    -H 'Accept: application/json' -H 'Content-Type: application/json' \
    -H "Authorization: Bearer $TOKEN" \
    -d '{"title": "Bought groceries", "content": "milk, eggs, bread"}'

# 4. (Alternative) Register a brand new account instead of using the seed:
